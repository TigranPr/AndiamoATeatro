...
public class UtenteService {
         ...
         if(numeroBigliettiComprati+numeroBiglietti>4){
             ...
             if(numeroBiglietti<=4){
                 for(Integer posto:nPostiDisponibili){
                         BigliettoRepository.insertBiglietto(new BigliettoRequest(idUtente,posto,IdSpettacolo));
                 }
    ...
    public List<Spettacolo> getSpettacoliSuggeriti(Integer idUtente) {
        ...
        spettacoliSuggeriti.addAll(SpettacoloRepository.getSpettacoliByGenere(generi.get(0)).stream()
        ...
        spettacoliSuggeriti.addAll(SpettacoloRepository.getSpettacoliByGenere(generi.get(1)).stream()
                .filter( spettacolo -> spettacolo.getDataOra().isAfter(LocalDate.now()
                        .atStartOfDay()) &&  spettacolo.getDataOra().isBefore(LocalDate.now().plusMonths(1).atStartOfDay())).toList());
        spettacoliSuggeriti.addAll(SpettacoloRepository.getSpettacoliByGenere(generi.get(2)).stream()
                .filter( spettacolo -> spettacolo.getDataOra().isAfter(LocalDate.now()
                        .atStartOfDay()) &&  spettacolo.getDataOra().isBefore(LocalDate.now().plusMonths(1).atStartOfDay())).toList());
